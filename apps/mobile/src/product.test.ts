...
describe("Stockholm demo capsule", () => {
  it("builds three reference-led Stockholm looks instead of generic minimal outfits", () => {
    const looks = demoOutfits("stockholm");
    const names = looks.flatMap((look) => look.items.map((item) => item.name));

    expect(looks).toHaveLength(3);
    expect(names.some((name) => /притал|кардиган-топ|поло/i.test(name))).toBe(true);
    expect(names.some((name) => /прямые джинсы|bootcut|клёш|спортивные брюки/i.test(name))).toBe(true);
    expect(names.some((name) => /угги|замшевые кроссовки|navy-деталями|высокие.*сапоги/i.test(name))).toBe(true);
    expect(names.some((name) => /замшевая сумка|city-сумка|сумка на плечо/i.test(name))).toBe(true);
    expect(names.every((name) => !/оверсайз|широкие джинсы|тренч/i.test(name))).toBe(true);
